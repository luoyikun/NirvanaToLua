...
namespace UiParticles
{
	...
	{
        private static readonly Vector2[] BillboardUVBuffer = new Vector2[4];
		...
		[SerializeField]
        [Tooltip("Whether support the UI clip.")]
        private bool m_SupportClip = true;
		...
		private static ParticleSystem.Particle[] ms_Particles;
        ...
        private void GetNodePath(Transform transform, ref string path)
        {
            if ("" != path)
            {
                path = "/" + path;
            ...
            if (null == transform.parent || null != transform.parent.GetComponent<Game.GameObjectAttach>())
            {
                return;
            }
            GetNodePath(transform.parent, ref path);
        }

        public override Material GetModifiedMaterial(Material baseMaterial)
        {
            if (this.m_SupportClip)
            {
		...
		{
			if (ms_Particles == null || ms_Particles.Length < ParticleSystem.main.maxParticles)
                ms_Particles = new ParticleSystem.Particle[ParticleSystem.main.maxParticles];
		}

		private void GenerateParticlesBillboards (VertexHelper vh)
		{
			InitParticlesBuffer ();
			int numParticlesAlive = ParticleSystem.GetParticles (ms_Particles);

			vh.Clear ();

			for (int i = 0; i < numParticlesAlive; i++) {
				DrawParticleBillboard (ms_Particles[i], vh);
			}
		}
